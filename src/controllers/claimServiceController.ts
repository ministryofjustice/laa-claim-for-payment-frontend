  ...
  try {
    ...
    if (response.status === "success" && response.data.length > minimumApiReponseLength) {
      const rows = response.data.map((claim) => [
        ...
        { text: claim.feeType },
        { text: formatClaimed(safeString(claim.claimed)), attributes: { "data-sort-value": claim.claimed }, classes: "govuk-table__cell--numeric" },
      ]);
      ...
      res.render("main/index.njk", {
        ...
        head: [
          ...
          { text: "Claimed", attributes: { "aria-sort": "none" }, classes: "govuk-table__header--numeric" },
        ],
      });
    } else {
