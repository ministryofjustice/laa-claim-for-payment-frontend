...
test.describe("How many clients retained page", () => {
  test("displays the how many clients retained page", async ({
    ...
    const howManyClientsRetainedPage = new HowManyClientsRetainedPage(
      page,
    ...
    await expect(howManyClientsRetainedPage.heading).toHaveText(
      "How many clients are retained?",
    );
    ...
    await expect(
      howManyClientsRetainedPage.saveAndContinueButton,
    ).toBeVisible();
  ...
  test("redirects to none route when None is selected", async ({ page }) => {
    ...
    await expect(page).toHaveURL(
      new RegExp(`/claims/${profitCostDraftClaim1Id}/poa/number-of-clients-start-of-case$`),
    );
  });
