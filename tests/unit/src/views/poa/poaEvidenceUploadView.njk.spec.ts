...
describe("views/main/poa/poaEvidenceUploadView.njk", () => {
  ...
  describe("no previously uploaded evidence", () => {
    ...
    it("doesn't renders an alert", () => {
      const alert = $(".moj-alert--information");

      expect(alert).to.have.length(0);
    });
  });

  describe("previously uploaded evidence but evidence not needed", () => {
    const claim = new Claim({
      id: claimId,
